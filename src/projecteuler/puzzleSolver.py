...
def moveNumber(chart, direction):
    ...
    if any(np.array(numberSwapIndex) < 0) | any(numberSwapIndex >= chart.shape):
        return chart

    return swap(chart, emptySpaceIndex, tuple(numberSwapIndex))
...
def puzzleSolver(chart):
    ...
    moveTrack.pop("x")
    visitedPaths = ["x"]
    distances = {k: manhattanDistance(v) for k, v in moveTrack.items()}
    while 0 not in distances.values():
        ...
        distances = {
            x: manhattanDistance(moveTrack[x])
            for x in moveTrack.keys()
            if x not in visitedPaths
        }  #
...
puzzleChart = np.array([[15, 14, 1, 6], [9, 11, 4, 12], [0, 10, 7, 3], [13, 8, 5, 2]])
